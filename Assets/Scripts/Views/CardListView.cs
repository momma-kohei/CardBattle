...
public class CardListView : MonoBehaviour // リストモデルを元にカードビューを利用しつつ仕様の場所にカードを表示するクラス
{
    ...
    public CardPresenter CardPresenter => _cardPresenter; // CardPresenterのプロパティ

    public void Start()
    {
        _cardPresenter = new CardPresenter(this, _handCardPrefab1); // 自信を渡してCardPresenterのインスタンスを生成
    }

    public void ShowHand(CardListModel _cardListModel) // 両者の手札にカードを表示するメソッド
    {
        // プレイヤー１の手札を表示
